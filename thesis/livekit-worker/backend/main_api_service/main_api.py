...
from custom_vad import vap_fasterwhisper_stt
...
from main_api_service import fasterwhisperclient, piperttsclient


def prewarm(proc: JobProcess):
    proc.userdata["vad"] = silero.VAD.load(min_silence_duration=0.7,min_speech_duration=0.3,prefix_padding_duration=0,activation_threshold=0.6)
    try:
        proc.userdata["stt"] = fasterwhisperclient.FasterWhisperClientSTT()
        # proc.userdata["stt"] = vap_fasterwhisper_stt.VAPFasterWhisperSTT()
        proc.userdata["tts"] = piperttsclient.PiperTTSClient()
    except Exception:
        import traceback
