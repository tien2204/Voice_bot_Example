...
from livekit.plugins.google.utils import (
    _build_gemini_fnc,
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            language (str, optional): The language(BCP-47 Code) to use for the API. supported languages - https://ai.google.dev/gemini-api/docs/live
    ...
    def update_options(
        ...
        *,
        ...
        for sess in self._sessions:
            sess.update_options(
                voice=self._opts.voice, temperature=self._opts.temperature
            )
...
class RealtimeSession(llm.RealtimeSession):
    def __init__(self, realtime_model: RealtimeModel) -> None:
        ...
        if self._opts.input_audio_transcription:
            self._opts.input_audio_transcription = None
        ...
        async with self._update_lock:
            ...
            if is_given(temperature) and self._opts.temperature != temperature:
                self._opts.temperature = (
                    temperature if is_given(temperature) else NOT_GIVEN
                )
                should_restart = True
    ...
    async def _transcription_task(self):
        if not self.transcription_queue or self.transcription_running:
            return
        self.transcription_running = True
        logger.warning("_transcription_task" * 20)
        print("_transcription_task" * 20)
        chunks: tp.List[Blob] = []
        while not self._msg_ch.closed or not self.transcription_queue.empty():
            try:
                chunk: Blob = await self.transcription_queue.get_nowait()
                chunks.append(chunk)
        ...
        resp = self._client.models.generate_content(
            ...
            contents=[
                "Transcribe audio. No comments. If there is no recognizable sound, returns empty string.",
                *[
        ...
        response_id = (
            list(self._current_generation.messages.keys())[0]
            if self._current_generation.messages
            else "no-response-id"
        )
        print(resp.text)
        if resp.text:
            self.emit(
                ...
                llm.InputTranscriptionCompleted(
                    item_id=response_id, transcript=resp.text
                ),
            )
                ...
                if isinstance(msg, LiveClientContent):
                    ...
                    for media_chunk in msg.media_chunks:
                        if self.transcription_queue:
                            self.transcription_queue.put_nowait(media_chunk)
    ...
    def _build_connect_config(self) -> LiveConnectConfig:
        ...
        return LiveConnectConfig(
            ...
            realtime_input_config=google.genai.types.RealtimeInputConfig(
                activity_handling=google.genai.types.ActivityHandling.NO_INTERRUPTION
            ),
    ...
    def _finalize_response(
        self, interrupted: bool = False, closed: bool = False
    ) -> None:
        if not self._current_generation:
            return
