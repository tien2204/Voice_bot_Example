...
class F5TTSWrapper:
    ...
    def __init__(
        ...
        if ckpt_path is None:
            ...
            if model_name == "F5TTS_v1_Pruned_14.yaml":
        ...
        config_path = str(
            os.path.join(
                os.path.dirname(__file__), "f5_tts", f"configs/{model_name}.yaml"
            )
        )
        ...
        if vocab_file is None:
            vocab_file = str(
                os.path.join(
                    os.path.dirname(__file__), "f5_tts", "infer/examples/vocab.txt"
                )
            )
        ...
        if self.use_duration_predictor and not self.has_duration_predictor:
            print(
                "Warning: Duration predictor requested but not found in model. Using fallback duration calculation."
            )
    ...
    def calculate_duration_with_predictor(
        self, text_tokens, text_lengths, local_speed=1.0
    ):
        """
        ...
        for text_batch in text_batches:
            ...
            if fix_duration is not None:
                ...
                if isinstance(final_text_list[0], str):
                    text_tokens = list_str_to_idx(
                        final_text_list, self.vocab_char_map
                    ).to(self.device)
                else:
                    text_tokens = torch.tensor(final_text_list, device=self.device)

                text_lengths = torch.tensor(
                    [len(t) for t in final_text_list], device=self.device
                )

                duration = self.calculate_duration_with_predictor(
                    text_tokens, text_lengths, local_speed
                )
