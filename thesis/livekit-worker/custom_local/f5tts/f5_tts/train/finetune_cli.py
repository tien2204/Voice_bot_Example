...
def parse_args():
    ...
    parser.add_argument(
        "--from_scratch",
        action="store_true",
        help="Train from scratch with initialized weights",
    )
    parser.add_argument(
...
def main():
    ...
    try:
        ...
        print(
            f"Warning: Using fallback paths '{base_data_path}' and '{base_ckpt_path}'. Ensure this is correct."
        )
    ...
    if (
        ...
        and (ckpt_path_source is None or not os.path.exists(ckpt_path_source))
    ):

        if ckpt_path_source is None:
            raise ValueError(
                f"Finetune selected but could not determine source checkpoint path for '{args.exp_name}'."
            )

        if not ckpt_path_source.startswith("hf://"):
            if not os.path.exists(ckpt_path_source):
                raise FileNotFoundError(
                    f"Determined source checkpoint path does not exist: {ckpt_path_source}"
                )
        else:
            print(
                "Source path is a Hugging Face identifier, existence will be checked by cached_path."
            )
    ...
    if (
        args.exp_name == "F5TTS_v1_Custom_Prune_14"
        or args.exp_name == "F5TTS_v1_Custom_Prune_12"
    ):
        ...
        if not os.path.isfile(extended_vocab_file_path):
            ...
            print(
                f"Extended vocab not found at primary path, checking fallback: {fallback_path}"
            )
            if os.path.isfile(fallback_path):
                extended_vocab_file_path = fallback_path
            else:
                raise FileNotFoundError(
                    f"Extended vocabulary file not found at expected locations: "
        ...
        print(
            f"Using EXTENDED vocab FILE for model init: {path_or_alias_for_get_tokenizer}"
        )

    elif args.tokenizer == "custom":
        if not args.tokenizer_path or not os.path.isfile(args.tokenizer_path):
            raise ValueError(
                f"Custom tokenizer selected, but --tokenizer_path '{args.tokenizer_path}' is invalid or not found."
            )
        path_or_alias_for_get_tokenizer = args.tokenizer_path
        tokenizer_type_for_init = "custom"
        print(
            f"Using custom tokenizer file from args: {path_or_alias_for_get_tokenizer}"
        )
    else:
        ...
        tokenizer_type_for_init = args.tokenizer
        print(
            f"Using default tokenizer associated with dataset '{args.dataset_name}' and type '{tokenizer_type_for_init}'"
        )

    print(
        f"Calling get_tokenizer with source='{path_or_alias_for_get_tokenizer}', type='{tokenizer_type_for_init}'"
    )
    ...
    if model_cls is None or model_cfg is None:
        raise RuntimeError("Model class or config was not set correctly.")
    print(
        f"Initializing {model_cls.__name__} with config: {model_cfg}, Vocab Size: {vocab_size}"
    )
    ...
    if args.use_duration_predictor and args.duration_loss_weight > 0:
        ...
        print(
            "Duration predictor requested but duration_loss_weight is 0. Setting up duration predictor anyway."
        )
        duration_predictor = DurationPredictor(vocab_size, 512, 32, 3, 0.5)
        if args.from_scratch:
            init_weights_tts(duration_predictor)
    ...
    if args.from_scratch:
        ...
        if args.learning_rate <= 1e-5:
            print(
                f"Note: Consider using a higher learning rate (e.g., 1e-4) for training from scratch."
            )
    ...
    if args.finetune and not args.from_scratch:
        ...
        file_checkpoint_for_trainer = os.path.join(
            target_dataset_ckpt_dir, target_basename
        )
        ...
        if not os.path.isfile(file_checkpoint_for_trainer):
            if not os.path.exists(ckpt_path_source):
                ...
                if ckpt_path_source.startswith("hf://"):
                    try:
                        ...
                        raise FileNotFoundError(
                            f"Source checkpoint '{ckpt_path_source}' not found locally and failed to resolve/download: {e}"
                        )
                else:
                    raise FileNotFoundError(
                        f"Source checkpoint for copying not found: {ckpt_path_source}"
                    )

            print(
                f"Copying checkpoint for finetune from '{ckpt_path_source}' to '{file_checkpoint_for_trainer}'"
            )
            try:
            ...
            print(
                f"Pretrained checkpoint already exists: {file_checkpoint_for_trainer}. Using existing."
            )
    elif args.from_scratch:

        if not os.path.isdir(target_dataset_ckpt_dir):
            print(
                f"Creating checkpoint directory for training from scratch: {target_dataset_ckpt_dir}"
            )
            os.makedirs(target_dataset_ckpt_dir, exist_ok=True)
    ...
    print(
        f"Loading dataset '{args.dataset_name}' using tokenizer type '{args.tokenizer}'"
    )
    ...
    if args.use_duration_predictor:
        print(
            "Training will use phoneme data from the arrow file for duration prediction"
        )
