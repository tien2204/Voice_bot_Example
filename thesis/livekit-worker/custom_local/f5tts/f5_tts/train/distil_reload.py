...
try:
    from f5_tts.infer.utils_infer import (
        ...
        sway_sampling_coef as default_sway_coef,
    )
except ImportError:
    print(
        "[Warning] f5_tts.infer.utils_infer not found. Sample logging requires manual inference parameters or defaults."
    )
    load_vocoder = None
...
try:
    ...
    print(
        "Please ensure the script is run from a directory where 'f5_tts' is importable, or the package is installed."
    )
    exit(1)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the full teacher model checkpoint (.pt or .safetensors)",
    )
    parser.add_argument(
        "--student_exp_name",
        type=str,
        default="F5TTS_v1_Custom_Prune_12",
        choices=["F5TTS_v1_Custom_Prune_14", "F5TTS_v1_Custom_Prune_12"],
        help="Experiment name identifying the STUDENT architecture",
    )
    parser.add_argument(
        "--student_init_ckpt_path",
        type=str,
        default=None,
        help="Path to the initial student checkpoint to start distillation from. If None, student starts from scratch or random init.",
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to save distillation checkpoints. Defaults to ckpts/{dataset_name}_distill_{student_exp_name}",
    )
    parser.add_argument(
        "--distill_loss_weight",
        type=float,
        default=0.5,
        help="Weight (alpha) for the distillation loss.",
    )
    parser.add_argument(
        "--spec_l1_weight",
        type=float,
    ...
    parser.add_argument(
        ...
        default="mse",
        choices=["mse", "l1"],
        help="Type of loss for comparing teacher/student flow predictions.",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Weight decay for STUDENT optimizer",
    )
    parser.add_argument(
        "--batch_size_per_gpu",
        type=int,
        default=4800,
        help="Batch size per GPU (frames or samples)",
    )
    parser.add_argument(
        "--batch_size_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Max sequences per batch (if batch_size_type='frame')",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--epochs", type=int, default=50, help="Number of distillation epochs"
    )
    parser.add_argument(
        "--num_warmup_updates",
        type=int,
        default=10000,
        help="Warmup updates for LR scheduler",
    )
    parser.add_argument(
        "--save_per_updates",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="-1 to keep all, 0 to not save intermediate, > 0 to keep last N",
    )
    parser.add_argument(
        "--last_per_updates",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="runs",
        help="Base directory for TensorBoard logs (if logger='tensorboard')",
    )
    parser.add_argument(
        "--bnb_optimizer", action="store_true", help="Use 8-bit Adam optimizer"
    )
    parser.add_argument(
        "--use_ema",
        action="store_true",
        default=True,
        help="Use Exponential Moving Average for student model",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--log_samples",
        action="store_true",
        help="Log inferenced audio samples during training.",
    )
    parser.add_argument(
        "--mixed_precision",
        type=str,
    ...
    parser.add_argument(
        "--resume_epoch", type=float, default=None, help="Resume from specific epoch"
    )
    parser.add_argument(
        "--from_scratch", action="store_true", default=False, help="From scratch"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Reference texts for sample generation",
    )
    parser.add_argument(
        "--ref_audio_paths",
        type=str,
        nargs="+",
        default=None,
        help="Paths to reference audio files",
    )
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Text prompts for reference samples",
    )
...
def load_model_checkpoint(model, ckpt_path, device, accelerator):
    ...
    try:
        if ckpt_path.endswith(".safetensors"):
            try:
                state_dict_raw = load_safetensors(ckpt_path, device=str(device))
            except Exception:
                accelerator.print(
                    f"Direct load to {device} failed, loading to CPU first."
                )
            ...
            for key in keys_to_try:
                if key in checkpoint_data and isinstance(checkpoint_data[key], dict):
                    state_dict_raw = checkpoint_data[key]
                    accelerator.print(f"Found model weights under key: '{key}'")
                    break
            if state_dict_raw is None:
                if isinstance(checkpoint_data, dict):
                    state_dict_raw = checkpoint_data
                    accelerator.print(
                        "Assuming the loaded .pt file root is the state_dict."
                    )
                else:
                    raise ValueError(
                        "Could not find a valid state_dict within the .pt file."
                    )
        else:
            raise ValueError(f"Unsupported checkpoint file extension: {ckpt_path}")

        if not isinstance(state_dict_raw, dict) or not state_dict_raw:
            raise ValueError("Loaded state_dict is not a valid dictionary or is empty.")

        cleaned_state_dict = {}
        prefixes_to_strip = ["module.", "_orig_mod.", "ema_model."]
        is_transformer_state_dict = all(
            not k.startswith("transformer.") for k in state_dict_raw.keys()
        ) and any(
            "attn" in k or "ff." in k or "embed" in k for k in state_dict_raw.keys()
        )
        ...
        if first_key:
            for p in prefixes_to_strip:
                if first_key.startswith(p) and sum(
                    1 for k in state_dict_raw if k.startswith(p)
                ) > 0.8 * len(state_dict_raw):
                    prefix_to_strip = p
                    break

        if prefix_to_strip:
            accelerator.print(
                f"Stripping prefix '{prefix_to_strip}' from state_dict keys."
            )
            prefix_len = len(prefix_to_strip)
            for k, v in state_dict_raw.items():
                cleaned_state_dict[
                    k[prefix_len:] if k.startswith(prefix_to_strip) else k
                ] = v
        else:
        ...
        if (
            ...
            and isinstance(model.transformer, nn.Module)
            and is_transformer_state_dict
        ):
            accelerator.print(
                "Detected transformer-only state_dict. Adding 'transformer.' prefix."
            )
            prefixed_state_dict = {}
            for k, v in cleaned_state_dict.items():
                prefixed_state_dict[
                    f"transformer.{k}" if not k.startswith("transformer.") else k
                ] = v
        ...
        if not final_state_dict:
            accelerator.print("ERROR: State dictionary became empty.")
            return False

        accelerator.print(
            f"Attempting to load state_dict into the full '{type(model).__name__}' model..."
        )
        ...
        if incompatible_keys.missing_keys:
            accelerator.print(f"Note: Missing keys: {incompatible_keys.missing_keys}")
        if incompatible_keys.unexpected_keys:
            accelerator.print(
                f"Note: Unexpected keys: {incompatible_keys.unexpected_keys}"
            )
        ...
        if "state_dict_raw" in locals():
            del state_dict_raw
        if "cleaned_state_dict" in locals():
            del cleaned_state_dict
        if "final_state_dict" in locals():
...
def generate_reference_samples(
    ...
    model_for_sampling = (
        ema_model.ema_model if ema_model is not None else unwrapped_model
    )
    ...
    with torch.inference_mode():
        ...
        for idx, ref_mel in enumerate(ref_mels):
            try:
                ...
                prompt_text_for_generation = (
                    ref_sample_text_prompts[idx]
                    if idx < len(ref_sample_text_prompts)
                ...
                accelerator.print(
                    f"Using combined CHARACTER text for model: {combined_text[:100]}..."
                )
                ...
                accelerator.print(
                    f"Decoding reference mel (shape: {ref_mel.shape}) for saving source audio"
                )
                ...
                target_duration = torch.full(
                    (batch_size,),
                    target_duration_frames,
                    device=ref_mel_for_sample.device,
                    dtype=torch.long,
                )
                accelerator.print(
                    f"Requesting target duration for sample: {target_duration.item()} frames"
                )
                ...
                if torch.isnan(generated_mel).any() or torch.isinf(generated_mel).any():
                    accelerator.print(
                        f"Error: NaNs or Infs detected in generated mel for reference sample {idx}. Skipping generation."
                    )
                    continue
                ...
                if generated_mel.shape[1] > ref_mel_frames:
                    mel_for_vocoder = (
                        generated_mel[:, ref_mel_frames:, :]
                        .permute(0, 2, 1)
                        .to(accelerator.device)
                    )
                    accelerator.print(
                        f"Using SLICED generated mel (after ref) for vocoder. Shape: {mel_for_vocoder.shape}"
                    )
                else:
                    accelerator.print(
                        f"Warning: Generated mel length ({generated_mel.shape[1]}) not longer than reference ({ref_mel_frames}). Output slice is empty."
                    )
                ...
                if mel_for_vocoder is not None and mel_for_vocoder.shape[-1] > 0:
                    ...
                    gen_path = (
                        f"{log_samples_path}/update_{global_update}_ref{idx}_gen.wav"
                    )
                    torchaudio.save(gen_path, gen_audio, target_sample_rate)
                    accelerator.print(
                        f"Saved generated sample (from sliced mel): {gen_path}"
                    )
                else:
                    accelerator.print(
                        "Skipping saving generated audio: sliced mel part is empty or generation too short."
                    )
                ...
                accelerator.print(
                    f"Completed processing for reference sample {idx} at update {global_update}"
                )
        ...
        if not is_last and keep_last_n != 0:
            try:
                ...
                for ckpt in checkpoints:
                    ...
                    if match:
                        numeric_checkpoints.append((int(match.group(1)), ckpt))
                    else:
                ...
                if keep_last_n > 0:
                    while len(sorted_ckpt_files) > keep_last_n:
                        ...
                        try:
                            os.remove(os.path.join(checkpoint_path, oldest_checkpoint))
                            print(
                                f"Removed old student checkpoint: {oldest_checkpoint}"
                            )
                        except OSError as e:
                            print(
                                f"Warning: Error removing old checkpoint {oldest_checkpoint}: {e}"
                            )
            except Exception as e:
...
def load_distill_checkpoint(
    accelerator, student_model, optimizer, scheduler, ema_model, checkpoint_path
):
    """
    ...
    if exists(checkpoint_path) and os.path.isdir(checkpoint_path):
        ...
        if os.path.exists(last_ckpt_path):
            ...
            accelerator.print(
                f"Found last student checkpoint: {latest_checkpoint_path}"
            )
        else:
            checkpoints = [
                f
            ...
            if checkpoints:
                ...
                if numeric_checkpoints:
                    ...
                    latest_checkpoint_path = os.path.join(
                        checkpoint_path, numeric_checkpoints[0][1]
                    )
                    accelerator.print(
                        f"Found latest training student checkpoint: {latest_checkpoint_path}"
                    )
                else:
                    accelerator.print("No numbered checkpoints found to resume from.")

    if latest_checkpoint_path is None or not os.path.exists(latest_checkpoint_path):
        accelerator.print(
            "No valid student checkpoint found for resuming distillation."
        )
        return 0, 0

    accelerator.print(
        f"Loading distillation state from: {latest_checkpoint_path} (before prepare)"
    )
    ...
    try:
        ...
        if model_sd:
            try:
                incompatible_keys = student_model.load_state_dict(
        ...
        if model_loaded:
            ...
            if opt_sd:
                try:
                    ...
                    accelerator.print(
                        "Continuing with fresh optimizer but keeping model weights"
                    )
            else:
            ...
            if optimizer_loaded:
                ...
                if sched_sd:
                    try:
                        ...
                        accelerator.print(
                            f"Warning: Could not load scheduler state: {e}"
                        )
            ...
            if ema_model is not None:
                ...
                if ema_sd:
                    try:
                        ...
                        accelerator.print(
                            f"Warning: Could not load EMA state: {e}. EMA will start fresh."
                        )
                else:
                    accelerator.print(
                        "Warning: ema_model_state_dict not found. EMA will start fresh."
                    )
        ...
        if model_loaded:
            ...
            if not optimizer_loaded:
                accelerator.print(
                    "Since optimizer couldn't be loaded, starting from update 0 with loaded model weights"
                )
            ...
            if optimizer_loaded:
                accelerator.print(
                    f"Resuming distillation from update {start_update}, epoch {start_epoch+1}"
                )
            else:
                accelerator.print(
                    f"Starting from update 0 with pre-trained model weights"
                )
        else:

            accelerator.print(
                "Could not load model state. Starting completely from scratch."
            )
            start_update = 0
            start_epoch = 0
        ...
        if "opt_sd" in locals():
            del opt_sd
        if "sched_sd" in locals():
            del sched_sd
        if ema_model is not None and "ema_sd" in locals():
            del ema_sd
        gc.collect()
...
def main():
    ...
    try:
        ...
        if not os.path.isdir(base_ckpt_path):
            raise FileNotFoundError("Ckpt path not found")
    except Exception as e:
        accelerator.print(
            f"[Warning] Error determining base paths: {e}. Using fallback './data', './ckpts'."
        )
        base_data_path = "./data"
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join(
            base_ckpt_path, f"{args.dataset_name}_distill_{args.student_exp_name}"
        )
    if accelerator.is_main_process:
    ...
    if args.student_exp_name in [
        "F5TTS_v1_Custom_Prune_14",
        "F5TTS_v1_Custom_Prune_12",
    ]:
        extended_vocab_file_path = os.path.join(
            base_data_path, f"{args.dataset_name}_{args.tokenizer}", "vocab.txt"
        )
        fallback_path = os.path.join(base_data_path, args.dataset_name, "vocab.txt")
        accelerator.print(
            f"Checking for extended vocab at: {extended_vocab_file_path} and fallback {fallback_path}"
        )
        if os.path.isfile(extended_vocab_file_path):
            path_or_alias_for_get_tokenizer = extended_vocab_file_path
            tokenizer_type_for_init = "custom"
        elif os.path.isfile(fallback_path):
            path_or_alias_for_get_tokenizer = fallback_path
            tokenizer_type_for_init = "custom"
        else:
            accelerator.print(
                f"[Warning] Extended vocab not found. Using default dataset alias '{args.dataset_name}'."
            )
    elif args.tokenizer == "custom":
        if not args.tokenizer_path or not os.path.isfile(args.tokenizer_path):
            accelerator.print(
                f"ERROR: Custom tokenizer path '{args.tokenizer_path}' invalid."
            )
            exit(1)
    ...
    accelerator.print(
        f"Using tokenizer source: '{path_or_alias_for_get_tokenizer}', type: '{tokenizer_type_for_init}'"
    )
    try:
        vocab_char_map, vocab_size = get_tokenizer(
            path_or_alias_for_get_tokenizer, tokenizer_type_for_init
        )
        accelerator.print(f"Tokenizer loaded. Vocab Size: {vocab_size}")
    except Exception as e:
        accelerator.print(f"ERROR loading tokenizer: {e}")
        exit(1)
    ...
    if max_len > 0:
        if len(ref_texts) > 0 and len(ref_texts) != max_len:
            accelerator.print(
                f"Warning: ref_texts length ({len(ref_texts)}) does not match other reference lists ({max_len}). Using available items only."
            )
        if len(ref_audio_paths) > 0 and len(ref_audio_paths) != max_len:
            accelerator.print(
                f"Warning: ref_audio_paths length ({len(ref_audio_paths)}) does not match other reference lists ({max_len}). Using available items only."
            )
        if len(ref_sample_text_prompts) > 0 and len(ref_sample_text_prompts) != max_len:
            accelerator.print(
                f"Warning: ref_sample_text_prompts length ({len(ref_sample_text_prompts)}) does not match other reference lists ({max_len}). Using available items only."
            )

    teacher_model_cls = DiT
    teacher_model_cfg = dict(
        dim=1024, depth=22, heads=16, ff_mult=2, text_dim=512, conv_layers=4
    )
    student_model_cls = DiT
    student_model_cfg = None
    if args.student_exp_name == "F5TTS_v1_Custom_Prune_14":
        student_model_cfg = dict(
            dim=1024, depth=14, heads=16, ff_mult=2, text_dim=512, conv_layers=4
        )
    elif args.student_exp_name == "F5TTS_v1_Custom_Prune_12":
        student_model_cfg = dict(
            dim=1024, depth=12, heads=16, ff_mult=2, text_dim=512, conv_layers=4
        )
    else:
        accelerator.print(
            f"ERROR: Unsupported student experiment name: {args.student_exp_name}"
        )
        exit(1)

    accelerator.print("Initializing Teacher Model (CPU)...")
    teacher_model = CFM(
        transformer=teacher_model_cls(
            **teacher_model_cfg, text_num_embeds=vocab_size, mel_dim=n_mel_channels
        ),
    ...
    student_model = CFM(
        transformer=student_model_cls(
            **student_model_cfg, text_num_embeds=vocab_size, mel_dim=n_mel_channels
        ),
        mel_spec_module=mel_spec_module,
        vocab_char_map=vocab_char_map,
    )
    ...
    teacher_load_success = load_model_checkpoint(
        teacher_model, args.teacher_ckpt_path, accelerator.device, accelerator
    )
    if not teacher_load_success:
        accelerator.print("Failed to load teacher checkpoint. Exiting.")
        return
    teacher_model.eval()
    ...
    if (
        ...
        and args.student_init_ckpt_path
        and os.path.exists(args.student_init_ckpt_path)
    ):
        if not os.path.isdir(args.output_dir):
            os.makedirs(args.output_dir, exist_ok=True)
        source_ckpt_basename = os.path.basename(args.student_init_ckpt_path)
        target_basename = (
            "pretrained_" + source_ckpt_basename
            if not source_ckpt_basename.startswith("pretrained_")
            else source_ckpt_basename
        )
    ...
    if (not args.from_scratch) and initial_student_ckpt_path_for_loading:
        accelerator.print(
            f"Loading initial Student state from: {initial_student_ckpt_path_for_loading} (before prepare)"
        )
        student_load_success = load_model_checkpoint(
            student_model,
            initial_student_ckpt_path_for_loading,
            torch.device("cpu"),
            accelerator,
        )
        if not student_load_success:
            accelerator.print(
                "[Warning] Failed to load initial student checkpoint. Starting fresh."
            )
        else:
            accelerator.print("Initial student state loaded successfully (CPU).")
            gc.collect()
    else:
        accelerator.print("No initial student checkpoint provided. Starting fresh.")

    accelerator.print("Checking for duration predictor in student model...")
    has_duration_predictor = (
        hasattr(student_model, "duration_predictor")
        and student_model.duration_predictor is not None
    )

    if not has_duration_predictor:
        accelerator.print(
            "Duration predictor not found in student model. Adding new duration predictor..."
        )
        try:
            ...
            accelerator.print(
                "[WARNING] Could not import DurationPredictor. Duration prediction will be disabled."
            )
        except Exception as e:
            accelerator.print(
                f"[WARNING] Error adding duration predictor: {e}. Duration prediction will be disabled."
            )
    else:
    ...
    if args.bnb_optimizer:
        try:
            ...
            accelerator.print("Using BNB 8-bit AdamW.")
        except ImportError:
            accelerator.print(
    ...
    if (
        args.use_duration_predictor
        and hasattr(student_model, "duration_predictor")
        and student_model.duration_predictor is not None
    ):
        ...
        optimizer = optimizer_cls(param_groups, betas=(0.9, 0.98), eps=1e-8)
        accelerator.print(
            f"Using optimizer with parameter groups - Duration predictor LR: {args.learning_rate * 3}, Others LR: {args.learning_rate}"
        )
    else:
        ...
        if optimizer_cls is AdamW:
            accelerator.print("Using standard AdamW.")

    accelerator.print(f"Loading dataset '{args.dataset_name}'...")
    try:
        train_dataset = load_dataset(
            args.dataset_name, args.tokenizer, mel_spec_kwargs=mel_spec_kwargs
        )
        accelerator.print(f"Dataset loaded. Size: {len(train_dataset)}")
    except Exception as e:
    ...
    if args.batch_size_type == "sample":
        ...
        temp_batch_sampler = DynamicBatchSampler(
            temp_sampler,
            frames_threshold=args.batch_size_per_gpu,
        ...
        accelerator.print(f"ERROR: Invalid batch_size_type: {args.batch_size_type}")
        exit(1)

    if actual_batches_per_epoch_per_gpu == 0 and len(train_dataset) > 0:
        accelerator.print(
            "[Warning] Calculated 0 batches per epoch per GPU. Check batch_size_per_gpu / frame threshold. Training might not proceed."
        )

    total_updates = (
        math.ceil(actual_batches_per_epoch_per_gpu / args.grad_accumulation_steps)
        * args.epochs
    )
    accelerator.print(
        f"Scheduler Calculation: Batches/Epoch/GPU={actual_batches_per_epoch_per_gpu}, Total Updates={total_updates}"
    )

    warmup_updates = args.num_warmup_updates
    decay_updates = max(0, total_updates - warmup_updates)
    accelerator.print(
        f"Scheduler Setup: Total Updates={total_updates}, Warmup Updates={warmup_updates}, Decay Updates={decay_updates}"
    )
    if warmup_updates >= total_updates and total_updates > 0:
        accelerator.print(f"[Warning] Warmup updates >= Total updates.")
        decay_updates = 0
    elif total_updates == 0:
        accelerator.print("[Warning] Total updates is 0. Setting warmup/decay to 0.")
        warmup_updates = 0
        decay_updates = 0

    warmup_scheduler = LinearLR(
        optimizer, start_factor=1e-8, end_factor=1.0, total_iters=max(0, warmup_updates)
    )
    decay_scheduler = CosineAnnealingLR(
        optimizer, T_max=max(1, decay_updates), eta_min=1e-8
    )
    ...
    if args.resume_epoch is not None:
        ...
        if start_epoch == 0:
            ...
            accelerator.print(
                f"Calculated resume epoch: {start_epoch} (based on update {start_update})"
            )
    ...
    accelerator.print(
        "Preparing model, optimizer, dataloader, scheduler with Accelerator..."
    )
        ...
        if logger_to_use == "wandb":
            try:
                ...
                wandb_run = wandb.init(
                    project=f"F5TTS_Distill_{args.dataset_name}",
                    name=run_name,
                    config=vars(args),
                    resume="allow",
                    id=wandb.util.generate_id() if start_update == 0 else None,
                )
                accelerator.print("Initialized WandB.")
            except ImportError:
                accelerator.print("[Warning] WandB not installed. Disabling WandB.")
                logger_to_use = None
            except Exception as e:
                accelerator.print(f"[Warning] WandB init failed: {e}. Disabling WandB.")
                logger_to_use = None
        elif logger_to_use == "tensorboard":
            try:
                tb_log_dir = os.path.join(args.output_dir, args.logging_dir, run_name)
                os.makedirs(tb_log_dir, exist_ok=True)
                tb_writer = SummaryWriter(log_dir=tb_log_dir)
                accelerator.print(f"Initialized TensorBoard: {tb_log_dir}")
            except Exception as e:
                accelerator.print(
                    f"[Warning] TensorBoard init failed: {e}. Disabling TensorBoard."
                )
                logger_to_use = None
    ...
    cfg_strength_sample = default_cfg_strength
    nfe_step_sample = default_nfe_step
    sway_sampling_coef_sample = default_sway_coef
    if args.log_samples and accelerator.is_main_process:
        if load_vocoder is None:
            accelerator.print(
                "[Warning] Cannot log samples: 'load_vocoder' failed import."
            )
            args.log_samples = False
        else:
            try:
                accelerator.print(f"Loading vocoder '{mel_spec_type}'...")
                vocoder = load_vocoder(vocoder_name=mel_spec_type)
                vocoder.to(accelerator.device)
                log_samples_path = os.path.join(args.output_dir, "samples")
                os.makedirs(log_samples_path, exist_ok=True)
                accelerator.print(f"Sample logging enabled: {log_samples_path}")
            except Exception as e:
                accelerator.print(
                    f"[Warning] Failed to load vocoder: {e}. Disabling sample logging."
                )
                args.log_samples = False
    ...
    if args.log_samples and accelerator.is_main_process and ref_audio_paths:
        ref_mels = preload_reference_mels(accelerator, ref_audio_paths, mel_spec_module)
        accelerator.print(
            f"Loaded {len(ref_mels)} reference mel spectrograms for consistent sampling"
        )
    ...
    if start_update > 0:
        ...
        accelerator.print(
            f"DIRECT CALCULATION: start_update={start_update}, forced_epoch={start_epoch+1}"
        )

        if actual_batches_per_epoch_per_gpu > 0:
            updates_per_epoch_per_gpu = math.ceil(
                actual_batches_per_epoch_per_gpu / args.grad_accumulation_steps
            )
            if updates_per_epoch_per_gpu > 0:
                ...
                skip_batches_in_epoch = (
                    skip_updates_in_epoch * args.grad_accumulation_steps
                )
                accelerator.print(
                    f"ORIGINAL CALC: start_update={start_update}, calculated_epoch={calculated_epoch+1}"
                )

                accelerator.print(
                    f"OVERRIDING epoch from {calculated_epoch+1} to {start_epoch+1}"
                )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.batch_size_type == "frame":
            if hasattr(train_dataloader, "batch_sampler") and hasattr(
                ...
                train_dataloader.batch_sampler.set_epoch(epoch + SEED)
            elif hasattr(train_dataloader, "sampler") and hasattr(
                train_dataloader.sampler, "set_epoch"
            ):
                train_dataloader.sampler.set_epoch(epoch + SEED)
        elif (
            args.batch_size_type == "sample"
            and hasattr(train_dataloader, "sampler")
        ...
        if is_resuming_epoch:
            accelerator.print(
                f"Skipping first {skip_batches_in_epoch} batches for epoch {epoch + 1}..."
            )
            effective_dataloader = accelerator.skip_first_batches(
                train_dataloader, skip_batches_in_epoch
            )
            progress_bar_initial = math.ceil(
                skip_batches_in_epoch / args.grad_accumulation_steps
            )

        total_batches_this_epoch_per_gpu = len(train_dataloader)
        total_updates_this_epoch_per_gpu = math.ceil(
            total_batches_this_epoch_per_gpu / args.grad_accumulation_steps
        )
        progress_bar = tqdm(
            total=total_updates_this_epoch_per_gpu,
        ...
        while processed_batches_in_epoch < total_batches_this_epoch_per_gpu:
            try:
                ...
                accelerator.print("[Warning] Batch iterator stopped.")
                break
            ...
            with accelerator.accumulate(student_model):
                ...
                try:
                    ...
                    if (
                        not hasattr(unwrapped_student, "vocab_char_map")
                        or not unwrapped_student.vocab_char_map
                    ):
                        raise ValueError("Student vocab_char_map missing.")
                    text_tensor_input = list_str_to_idx(
                        text_list_input,
                        unwrapped_student.vocab_char_map,
                        padding_value=-1,
                    ).to(accelerator.device)
                except Exception as e:
                    accelerator.print(f"ERROR preparing batch data: {e}")
                ...
                frac_lengths = (
                    torch.zeros((batch_size,), device=device).float().uniform_(0.7, 1.0)
                )
                rand_span_mask = mask_from_frac_lengths(mel_lengths, frac_lengths).to(
                    device
                )
                if exists(mask):
                    rand_span_mask &= mask
                ...
                t_unsqueezed = time.unsqueeze(-1).unsqueeze(-1)
                xt = (1 - t_unsqueezed) * x0 + t_unsqueezed * x1
                target_flow = x1 - x0
                cond = torch.where(
                    rand_span_mask[..., None], torch.zeros_like(x1, device=device), x1
                )

                with torch.no_grad():
                    teacher_model.eval()
                    try:
                        teacher_pred_flow = teacher_model.transformer(
                            x=xt,
                            cond=cond,
                            text=text_tensor_input,
                            time=time,
                            drop_audio_cond=False,
                            drop_text=False,
                        )
                    except Exception as e:
                        accelerator.print(f"ERROR Teacher forward: {e}")
                        processed_batches_in_epoch += 1
                        continue
                ...
                drop_audio_cond = (
                    random.random() < unwrapped_student_model.audio_drop_prob
                )
                drop_text = random.random() < unwrapped_student_model.cond_drop_prob
                if drop_text:
                    drop_audio_cond = True
                try:
                    student_pred_flow = unwrapped_student_model.transformer(
                        x=xt,
                        cond=cond,
                        text=text_tensor_input,
                        time=time,
                    ...
                    accelerator.print(f"ERROR Student forward: {e}")
                    processed_batches_in_epoch += 1
                    continue

                try:
                    student_loss_full = F.mse_loss(
                        student_pred_flow, target_flow, reduction="none"
                    )
                    masked_student_loss = student_loss_full * rand_span_mask.unsqueeze(
                        -1
                    )
                    student_loss = (
                        masked_student_loss.sum() / rand_span_mask.sum().clamp(min=1)
                    )

                    if args.distill_loss_type == "mse":
                        distill_loss_full = F.mse_loss(
                            student_pred_flow,
                            teacher_pred_flow.detach(),
                            reduction="none",
                        )
                    elif args.distill_loss_type == "l1":
                        distill_loss_full = F.l1_loss(
                            student_pred_flow,
                            teacher_pred_flow.detach(),
                            reduction="none",
                        )
                        ...
                        raise ValueError(
                            f"Unsupported distill_loss_type: {args.distill_loss_type}"
                        )

                    if args.spec_l1_weight > 0.0:
                        if args.spec_l1_weight > 0.0:
                            spec_l1_full = F.l1_loss(
                                student_pred_flow,
                                teacher_pred_flow.detach(),
                                reduction="none",
                            )
                            masked_spec = spec_l1_full * rand_span_mask.unsqueeze(-1)
                            spec_l1 = masked_spec.sum() / rand_span_mask.sum().clamp(
                                min=1
                            )
                        else:
                    ...
                    masked_distill_loss = distill_loss_full * rand_span_mask.unsqueeze(
                        -1
                    )
                    distill_loss = (
                        masked_distill_loss.sum() / rand_span_mask.sum().clamp(min=1)
                    )
                    ...
                    total_loss = (
                        ...
                        + alpha * distill_loss
                        + spec_l1 * args.spec_l1_weight
                    )

                    if (
                        args.use_duration_predictor
                        and hasattr(unwrapped_student_model, "duration_predictor")
                        and unwrapped_student_model.duration_predictor is not None
                        and "attn" in batch
                    ):
                        text_tokens = text_tensor_input
                        b, nt = text_tokens.shape
                        text_lengths = batch.get(
                            "text_lengths", torch.full((b,), nt, device=device)
                        )
                        attn = batch["attn"]

                        range_tensor = torch.arange(nt, device=device).unsqueeze(0)
                        text_tokens_mask = (
                            range_tensor < text_lengths.unsqueeze(1)
                        ).int()

                        w = attn.sum(dim=2)
                        logw_ = torch.log(w + 1e-6) * text_tokens_mask

                        logw = unwrapped_student_model.duration_predictor(
                            text_tokens, text_tokens_mask
                        )
                        ...
                        duration_loss_weight = getattr(
                            args, "duration_loss_weight", 0.5
                        )
                    ...
                    if torch.isnan(total_loss) or torch.isinf(total_loss):
                        accelerator.print(
                            f"[ERROR] NaN/Inf loss at update {global_update}! Skipping."
                        )
                        optimizer.zero_grad()
                        processed_batches_in_epoch += 1
                        continue
                except Exception as e:
                    accelerator.print(f"ERROR Loss calculation: {e}")
                    processed_batches_in_epoch += 1
                    continue
                ...
                if accelerator.sync_gradients:
                    ...
                    if accelerator.is_main_process:
                        ...
                        log_data = {
                            ...
                            "loss/student": student_loss.item(),
                            "loss/distill": distill_loss.item(),
                            "learning_rate": lr,
                        }
                        if grad_norm is not None:
                            log_data["gradient_norm"] = grad_norm.item()

                        if args.logger == "wandb" and wandb_run:
                            try:
                                wandb.log(log_data, step=global_update)
                            except Exception as e:
                                accelerator.print(f"[Warning] WandB log failed: {e}")
                        elif args.logger == "tensorboard" and tb_writer:
                            try:
                                for key, value in log_data.items():
                                    tb_writer.add_scalar(
                                        key, value, global_step=global_update
                                    )
                            except Exception as e:
                                accelerator.print(
                                    f"[Warning] TensorBoard log failed: {e}"
                                )
                        ...
                        if grad_norm is not None:
                            postfix_data["grad"] = f"{grad_norm.item():.2f}"
                        progress_bar.set_postfix(**postfix_data)

                    if accelerator.is_main_process:
                        if (
                            global_update > 0
                            and global_update % args.save_per_updates == 0
                        ):
                            ...
                            if args.log_samples and vocoder is not None:
                                try:
                                    accelerator.print(
                                        f"Generating samples for update {global_update}..."
                                    )
                                    model_for_sampling = (
                                        ema_model.ema_model
                                        if args.use_ema and ema_model is not None
                                        else accelerator.unwrap_model(student_model)
                                    )
                                    model_for_sampling.eval()
                                    ref_audio_len = mel_lengths[0].item()
                                    first_text = text_list_input[0]
                                    first_text_str = (
                                        "".join(first_text)
                                    ...
                                    target_sr = (
                                        model_for_sampling.mel_spec.target_sample_rate
                                    )

                                    with torch.inference_mode():
                                        generated_mel, _ = model_for_sampling.sample(
                                            cond=mel_input[0][:ref_audio_len].unsqueeze(
                                                0
                                            ),
                                            text=infer_text,
                                            duration=ref_audio_len * 2,
                                            steps=nfe_step_sample,
                                            cfg_strength=cfg_strength_sample,
                                            sway_sampling_coef=sway_sampling_coef_sample,
                                        )
                                        generated_mel = generated_mel.to(torch.float32)
                                        gen_mel_only = (
                                            generated_mel[:, ref_audio_len:, :]
                                            if generated_mel.shape[1] > ref_audio_len
                                            else generated_mel
                                        )
                                        gen_mel_vocoder = gen_mel_only.permute(
                                            0, 2, 1
                                        ).to(accelerator.device)
                                        ref_mel_vocoder = (
                                            batch["mel"][0]
                                        ...
                                        if mel_spec_type == "vocos":
                                            gen_audio, ref_audio = (
                                                vocoder.decode(gen_mel_vocoder).cpu(),
                                                vocoder.decode(ref_mel_vocoder).cpu(),
                                            )
                                        else:
                                            gen_audio, ref_audio = torch.zeros(
                                                (1, 1)
                                            ), torch.zeros((1, 1))

                                    save_path_gen = os.path.join(
                                        log_samples_path,
                                        f"update_{global_update}_gen.wav",
                                    )
                                    save_path_ref = os.path.join(
                                        log_samples_path,
                                        f"update_{global_update}_ref.wav",
                                    )
                                    torchaudio.save(save_path_gen, gen_audio, target_sr)
                                    torchaudio.save(save_path_ref, ref_audio, target_sr)
                                    accelerator.print(
                                        f"Saved sample audio to {log_samples_path}"
                                    )
                                    ...
                                    if ref_mels:
                                        try:
                                            generate_reference_samples(
                                                ...
                                                nfe_step_sample,
                                                cfg_strength_sample,
                                                sway_sampling_coef_sample,
                                            )
                                        except Exception as e:
                                            accelerator.print(
                                                f"Warning: Error generating reference samples: {e}"
                                            )
                                            import traceback
                                    ...
                                    accelerator.print(
                                        f"[Warning] Sample generation failed: {sample_err}"
                                    )
                                    student_model.train()

                        if (
                            global_update > 0
                            and global_update % args.last_per_updates == 0
                        ):
                            save_distill_checkpoint(
                                accelerator,
                                student_model,
        ...
        if accelerator.is_main_process:
            accelerator.print(
                f"Epoch {epoch + 1} finished. Saving epoch end checkpoint."
            )
            save_distill_checkpoint(
                accelerator,
                student_model,
    ...
    if accelerator.is_main_process:
        ...
        if args.logger == "wandb" and wandb_run:
            try:
                wandb.finish()
                accelerator.print("WandB logger finished.")
            except Exception as e:
                accelerator.print(f"[Warning] Error closing WandB: {e}")
        elif args.logger == "tensorboard" and tb_writer:
            try:
                tb_writer.close()
                accelerator.print("TensorBoard logger closed.")
            except Exception as e:
                accelerator.print(f"[Warning] Error closing TensorBoard writer: {e}")
