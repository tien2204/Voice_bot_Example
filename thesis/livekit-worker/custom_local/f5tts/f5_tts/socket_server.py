...
class TTSStreamingProcessor:
    ...
    def update_reference(self, ref_audio, ref_text):
        ...
        self.max_chars = int(
            ref_text_byte_len / (ref_audio_duration) * (25 - ref_audio_duration)
        )
        self.few_chars = int(
    ...
    def generate_stream(self, text, conn):
        ...
        if self.first_package:
            text_batches = (
                chunk_text(text_batches[0], max_chars=self.few_chars) + text_batches[1:]
            )
            text_batches = (
                chunk_text(text_batches[0], max_chars=self.min_chars) + text_batches[1:]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dtype", default=torch.float32, help="Data type to use for model inference"
    )
