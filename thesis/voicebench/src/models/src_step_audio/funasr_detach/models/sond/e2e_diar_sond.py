...
class DiarSondModel(FunASRModel):
    ...
    def speaker_discrimination_loss(
        ...
        mask = torch.matmul(profile_mask, profile_mask.transpose(1, 2))  
    ...
    def concate_speech_ivc(speech: torch.Tensor, ivc: torch.Tensor) -> torch.Tensor:
        ...
        speech = speech.unsqueeze(dim=1)  
        speech = speech.expand(-1, nn, -1, -1)  
        ivc = ivc.unsqueeze(dim=2)  
        ivc = ivc.expand(-1, -1, tt, -1)  
        sd_in = torch.cat([speech, ivc], dim=3)  
        return sd_in
